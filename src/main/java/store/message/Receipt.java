    ...
    private static String formatHeader(ReceiptDto receiptDto) {
        return HEADER.format() +
                ...
                receiptDto.getProducts().stream()
                        .map(product -> ITEM.format(product.getName(), product.getQuantity(), product.getTotalPrice()))
    ...
    private static String formatPrices(ReceiptDto receiptDto) {
        return FOOTER.format() +
                TOTAL.format(receiptDto.getTotalPriceAndQuantity().getTotalQuantity(),
                        receiptDto.getTotalPriceAndQuantity().getTotalPrice()) +
                EVENT_DISCOUNT.format(-receiptDto.getPromotionDiscount()) +
                MEMBERSHIP_DISCOUNT.format(-receiptDto.getMembershipDiscount()) +
                FINAL_AMOUNT.format(receiptDto.getTotalPayment());
    }
